...
var Depuck1,ctDepuck1=0,speedTempDepuck1=0,secDepuck1=0,stopCountDepuck1=0,flagStopDepuck1=0,flagPrintDepuck1=0,speedDepuck1=0,timeDepuck1=0;
var actualDepuck1=0,stateDepuck1=0;
...
var Depuck2,ctDepuck2=0,speedTempDepuck2=0,secDepuck2=0,stopCountDepuck2=0,flagStopDepuck2=0,flagPrintDepuck2=0,speedDepuck2=0,timeDepuck2=0;
var actualDepuck2=0,stateDepuck2=0;
...
function idle(){
  ...
  for ( k=0;k<files.length;k++){//Verificar los archivos
    var stats = fs.statSync("/home/oee/Pulse/BYD_L12_LOGS/"+files[k]);
  ...
  if(secPubNub>=60*5){
    ...
    client.readHoldingRegisters(0,99).then(function(resp){
        var statesFiller              = switchData(resp.register[0],resp.register[1]),
            ...
            statesDepuck1             = switchData(resp.register[4],resp.register[5]),
            ...
            statesTapper               = switchData(resp.register[12],resp.register[13]),
            ...
            //Filler -------------------------------------------------------------------------------------------------------------
              ...
              if(flagPrintFiller==1){
                  for(var key in Filler){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L12_LOGS/pol_byd_Filler_L12.log","tt="+timeFiller+",var="+key+",val="+Filler[key]+"\n");
                  }
                  flagPrintFiller=0;
              }
            //Filler -------------------------------------------------------------------------------------------------------------
            //Capper -------------------------------------------------------------------------------------------------------------
              ...
              if(flagPrintCapper==1){
                  for(var key in Capper){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L12_LOGS/pol_byd_Capper_L12.log","tt="+timeCapper+",var="+key+",val="+Capper[key]+"\n");
                  }
                  flagPrintCapper=0;
              }
            //Capper -------------------------------------------------------------------------------------------------------------
            //Depuck1 -------------------------------------------------------------------------------------------------------------
              ctDepuck1 = joinWord(resp.register[27],resp.register[26]);
                if(flagONS3===0){
              ...
              if(flagPrintDepuck1==1){
                  for(var key in Depuck1){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L12_LOGS/pol_byd_Depuck1_L12.log","tt="+timeDepuck1+",var="+key+",val="+Depuck1[key]+"\n");
                  }
                  flagPrintDepuck1=0;
              }
            //Depuck1 -------------------------------------------------------------------------------------------------------------
            //Labeller -------------------------------------------------------------------------------------------------------------
              ...
              if(flagPrintLabeller==1){
                  for(var key in Labeller){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L12_LOGS/pol_byd_Labeller_L12.log","tt="+timeLabeller+",var="+key+",val="+Labeller[key]+"\n");
                  }
                  flagPrintLabeller=0;
              }
            //Labeller -------------------------------------------------------------------------------------------------------------
            //Depuck2 -------------------------------------------------------------------------------------------------------------
              ...
              if(flagPrintDepuck2==1){
                  for(var key in Depuck2){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L12_LOGS/pol_byd_Depuck2_L12.log","tt="+timeDepuck2+",var="+key+",val="+Depuck2[key]+"\n");
                  }
                  flagPrintDepuck2=0;
              }
            //Depuck2 -------------------------------------------------------------------------------------------------------------
            //Shrinkwrapper -------------------------------------------------------------------------------------------------------------
              ctShrinkwrapper = joinWord(resp.register[39],resp.register[38]);
                if(flagONS6===0){
              ...
              if(flagPrintShrinkwrapper==1){
                  for(var key in Shrinkwrapper){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L12_LOGS/pol_byd_Shrinkwrapper_L12.log","tt="+timeShrinkwrapper+",var="+key+",val="+Shrinkwrapper[key]+"\n");
                  }
                  flagPrintShrinkwrapper=0;
              }
            //Shrinkwrapper -------------------------------------------------------------------------------------------------------------
            //Tapper -------------------------------------------------------------------------------------------------------------
              ctTapper = joinWord(resp.register[41],resp.register[40]);
                if(flagONS7===0){
              ...
              if(flagPrintTapper==1){
                  for(var key in Tapper){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L12_LOGS/pol_byd_Tapper_L12.log","tt="+timeTapper+",var="+key+",val="+Tapper[key]+"\n");
                  }
                  flagPrintTapper=0;
              }
            //Tapper -------------------------------------------------------------------------------------------------------------
            //Checkweigher -------------------------------------------------------------------------------------------------------------
              ctCheckweigher = joinWord(resp.register[45],resp.register[44]);
                if(flagONS8===0){
              ...
              if(flagPrintCheckweigher==1){
                  for(var key in Checkweigher){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L12_LOGS/pol_byd_Checkweigher_L12.log","tt="+timeCheckweigher+",var="+key+",val="+Checkweigher[key]+"\n");
                  }
            ...
            //Printer -------------------------------------------------------------------------------------------------------------
              ...
              if(flagPrintPrinter==1){
                  for(var key in Printer){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L12_LOGS/pol_byd_Printer_L12.log","tt="+timePrinter+",var="+key+",val="+Printer[key]+"\n");
                  }
                  flagPrintPrinter=0;
              }
            //Printer -------------------------------------------------------------------------------------------------------------
            //Paletizer -------------------------------------------------------------------------------------------------------------
              ctPaletizer = joinWord(resp.register[55],resp.register[54]);
                if(flagONS10===0){
              ...
              if(flagPrintPaletizer==1){
                  for(var key in Paletizer){
                      fs.appendFileSync("/home/oee/Pulse/BYD_L12_LOGS/pol_byd_Paletizer_L12.log","tt="+timePaletizer+",var="+key+",val="+Paletizer[key]+"\n");
                  }
                  flagPrintPaletizer=0;
              }
            //Paletizer -------------------------------------------------------------------------------------------------------------
          //Barcode -------------------------------------------------------------------------------------------------------------
          if(resp.register[59]==0&&resp.register[60]==0&&resp.register[61]==0&&resp.register[62]==0&&resp.register[63]==0&&resp.register[64]==0&&resp.register[65]==0&&resp.register[76]==0){
            Barcode='0';
          }else {
            var dig1=hex2a(assignment(resp.register[59]).toString(16));
            var dig2=hex2a(assignment(resp.register[60]).toString(16));
            var dig3=hex2a(assignment(resp.register[61]).toString(16));
            var dig4=hex2a(assignment(resp.register[62]).toString(16));
            var dig5=hex2a(assignment(resp.register[63]).toString(16));
            var dig6=hex2a(assignment(resp.register[64]).toString(16));
            var dig7=hex2a(assignment(resp.register[65]).toString(16));
            var dig8=hex2a(assignment(resp.register[66]).toString(16));
          ...
          if(isNaN(Barcode)){
	        ...
	        if(secBarcode>=60&&!isNaN(Barcode)){
              writedataBarcode(Barcode,"pol_byd_Barcode_L12.log");
              secBarcode=0;
          }
          secBarcode++;
          //Barcode -------------------------------------------------------------------------------------------------------------
          //EOL --------------------------------------------------------------------------------------------------------------------
          if(secEOL>=60){
            fs.appendFileSync("../BYD_L12_LOGS/pol_byd_EOL_L12.log","tt="+Date.now()+",var=EOL"+",val="+Paletizer.CPQI+"\n");
            secEOL=0;
          }
          secEOL++;
          //EOL --------------------------------------------------------------------------------------------------------------------
...
var writedata = function (varJson,nameFile){
    ...
    for(var key in varJson){
        fs.appendFileSync("/home/pi/Pulse/BYD_L12_LOGS/"+nameFile,"tt="+timet+",var="+key+",val="+varJson[key]+"\n");
    }
};

var writedataBarcode = function (barcode,nameFile){
    var timet=Date.now();
    fs.appendFileSync("../BYD_L12_LOGS/"+nameFile,"tt="+timet+",var=bc"+",val="+barcode+"\n");
};
